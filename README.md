...
The version is pinned in `docker-compose.yaml` (the `image:` line ends in `42.20.3-release`), so it won't change on its own. That's on purpose: a surprise Build 42 patch can break your save. To update when you're ready, back up `server-data/Zomboid` first, bump that tag to the newer build, then run:
